...
object Exercises {
	...
	def prettyBooleanFormatter3(x: Any): String = {
		x.getClass.getSimpleName match {
			case "Boolean" => if (x.asInstanceOf[Boolean]) "правда" else "ложь"
			case _ => x.toString
		}
	}
	...
	def sum2(x: Int, y: Int): Int = sumIntegers(IterableCaseClass(x, y))
	
	def sum3(x: Int, y: Int): Int = sumIntegers(IterableClass(x, y).getParametersSequence)
	
	
	case class IterableCaseClass[T](x: T, y: T) extends Iterable[T] {
		override def iterator: Iterator[T] = Iterator(x, y)
	}
	
	case class IterableClass[T](x: T, y: T) {
		def getParametersSequence: Seq[T] ={
			var parameters = Seq[T]()
			val thisClass = this.getClass
			
			for (field <- thisClass.getDeclaredFields)
				parameters :+= field.get(this).asInstanceOf[T]
